...
func (p *Proxy) ServeTun(ctx context.Context, r io.Reader, w io.Writer) {
	...
	if counter := p.opts.downloadCounter; counter != nil {
	...
	if err := common.PackBodyConfig(buf, p.opts.connectAddr); err != nil {
		log.Println("packBodyConfig err", err)
		return
	}
	log.Printf("Write CmdConfig, connectAddr %s", p.opts.connectAddr)
	...
	s := tcp.NewServer(
		tcp.WithListenAddress(p.opts.listenAddr),
	...
	log.Printf("tcp server listen on %s", p.opts.listenAddr)
